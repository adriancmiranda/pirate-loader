const { is, typify, stringify } = require('describe-type');
const PLError = require('./error');
...
function parseAssert(errors, assert) {
	...
	if (is.not(Object, assert.data)) {
		throw PLError.UNKN0WN_SCHEM4(describe(assert));
	}
	if ('default' in assert.data && is.not(assert.data.type, assert.data.default)) {
		throw PLError.CONFL1CT_TYPE(describe(assert));
	}
	if (assert.data.required && !validateAssert(assert.data)) {
		throw PLError.INVALID_SCHEM4(describe(assert));
	}
	return assert.data;
}
...
function parseConfig(errors, assert, config) {
	...
	throw PLError.UNEXPECTED_TYPE(describe(assert), describe(config));
}
