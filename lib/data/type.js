...
exports.name = (value, write) => {
	const type = exports.of(value);
	if (type === 'Object' || value === undefined || value === null) {
		return type;
	}
	return Object(value).name || (write && type === 'String' ?
		value.replace(varName, '_') : exports.constructorNameOf(value)
	);
};

exports.typify = (expected, write) => {
	if (Array.isArray(expected) && expected.length > 0) {
		return expected.map(val => exports.typify(val, write)).join('|');
	}
	return exports.name(expected, write);
};
